...
#ifndef FINGERPRINT_SENSOR_LIBRARY_H_
#define FINGERPRINT_SENSOR_LIBRARY_H_
...
typedef struct
{
    ...
    uint32_t* address;     ///< 32-bit Fingerprint sensor address
...
#define GET_CMD_PACKET(...)                                                          \
    ...
    Init_Fingerprint_Packet(&packet, FINGERPRINT_COMMANDPACKET, sizeof(data), data); \
...
void Init_Fingerprint_Packet(Fingerprint_Packet *packet, uint8_t type, uint16_t length, uint8_t *data)
{
...
uint16_t fingerID;
/// The confidence of the fingerFastSearch() match, higher numbers are more confident
uint16_t confidence;
...
#endif /* FINGERPRINT_SENSOR_LIBRARY_H_ */
