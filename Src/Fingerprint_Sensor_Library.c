...
{
    HAL_UART_Transmit(mySerial, (uint8_t *)packet->start_code, 2, HAL_MAX_DELAY);
    HAL_UART_Transmit(mySerial, (uint8_t *)packet->address, 4, HAL_MAX_DELAY);
    HAL_UART_Transmit(mySerial, (uint8_t *)packet->type, 1, HAL_MAX_DELAY);

    uint16_t wire_length = *(packet->length) + 2;
    HAL_UART_Transmit(mySerial, (uint8_t *)(&wire_length), 2, HAL_MAX_DELAY);

    uint16_t sum = wire_length + *(packet->type);
    for (uint8_t i = 0; i < *(packet->length); i++)
    {
        HAL_UART_Transmit(mySerial, (uint8_t *)(&packet->data[i]), 1, HAL_MAX_DELAY);
        sum += packet->data[i];
    }

    HAL_UART_Transmit(mySerial, (uint8_t*)&sum, 2, HAL_MAX_DELAY);
